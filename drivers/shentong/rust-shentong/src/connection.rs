...
impl Connection {
    ...
    /// ```
    ///
    pub fn prepare(&self, sql: &str, params: &[StmtParam]) -> Result<Statement<'_>> {
    ...
    pub fn query(&self, sql: &str, params: &[&dyn ToSql]) -> Result<ResultSet<'_, Row>> {
    ...
    ///
    /// [Query Methods]: https://github.com/kubo/rust-oracle/blob/master/docs/query-methods.md
    pub fn query_as<T>(&self, sql: &str, params: &[&dyn ToSql]) -> Result<ResultSet<'_, T>>
    where
        T: RowValue,
    {
        let mut stmt = self.statement(sql).build()?;
    ...
    /// # Ok::<(), Error>(())
    /// ```
    pub fn execute(&self, sql: &str, params: &[&dyn ToSql]) -> Result<Statement<'_>> {
        let mut stmt = self.statement(sql).build()?;
        stmt.execute(params)?;
        Ok(stmt)
    }
    ...
    /// # Ok::<(), Error>(())
    /// ```
    pub fn execute_named(&self, sql: &str, params: &[(&str, &dyn ToSql)]) -> Result<Statement<'_>> {
        let mut stmt = self.statement(sql).build()?;
