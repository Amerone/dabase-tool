        ...
        uint16_t *charsetId, char *encoding, dpiError *error)
{
    *charsetId = 0;
    ...
    // aci.dll may not populate OCI_ATTR_CHARSET_ID/NCHARSET_ID; treat 0
    // as UTF-8 (the charset we always use to create the environment).
    if (*charsetId == 0)
        *charsetId = DPI_CHARSET_ID_UTF8;
    // ShenTong ACI defines ACI_CHARSET_UTF8=871 (not 873). Map to ODPI's UTF-8.
    if (*charsetId == 871)
        *charsetId = DPI_CHARSET_ID_UTF8;
