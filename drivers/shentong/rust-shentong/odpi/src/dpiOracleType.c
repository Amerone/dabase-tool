        ...
        uint32_t handleType, dpiDataTypeInfo *info, dpiError *error)
{
    ...
    if (info->ociTypeCode != DPI_SQLT_CHR &&
        ...
        // ShenTong ACI does not support ATTR_CHARSET_FORM on describe handles.
        // Default to IMPLICIT if the call fails.
        if (dpiOci__attrGet(handle, handleType, (void*) &charsetForm, 0,
                DPI_OCI_ATTR_CHARSET_FORM, NULL, error) < 0)
            charsetForm = DPI_SQLCS_IMPLICIT;
    }
    ...
    // ShenTong ACI does not support ATTR_CHAR_SIZE on describe handles; use byte size as fallback.
    if (oracleType && oracleType->isCharacterData &&
            oracleType->sizeInBytes == 0) {
        if (dpiOci__attrGet(handle, handleType, (void*) &ociSize, 0,
                DPI_OCI_ATTR_CHAR_SIZE, NULL, error) < 0) {
            // Fallback: assume 1 byte per character
            info->sizeInChars = info->dbSizeInBytes;
        } else {
