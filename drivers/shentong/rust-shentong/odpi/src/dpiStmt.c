        ...
        const char *name, uint32_t nameLength, dpiError *error)
{
    ...
    // ShenTong ACI does not support ATTR_MAXDATA_SIZE on bind handles; ignore failures.
    if (var->type->sizeInBytes == 0 && !var->isDynamic) {
        dpiOci__attrSet(bindHandle, DPI_OCI_HTYPE_BIND,
...
static int dpiStmt__fetch(dpiStmt *stmt, dpiError *error)
{
    ...
    // perform fetch
    fprintf(stderr, "SHENTONG_DEBUG: dpiStmt__fetch() calling stmtFetch2, fetchArraySize=%u\n",
            stmt->fetchArraySize);
    int fetchStatus = dpiOci__stmtFetch2(stmt, stmt->fetchArraySize, DPI_MODE_FETCH_NEXT, 0,
            error);
    fprintf(stderr, "SHENTONG_DEBUG: dpiStmt__fetch() stmtFetch2 returned %d\n", fetchStatus);
    if (fetchStatus < 0)
        return DPI_FAILURE;
    ...
    if (dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT,
        ...
        return DPI_FAILURE;
    fprintf(stderr, "SHENTONG_DEBUG: dpiStmt__fetch() bufferRowCount=%u\n",
            stmt->bufferRowCount);
...
int dpiStmt__init(dpiStmt *stmt, dpiError *error)
{
    ...
    // Shentong ACI does not support STMT_IS_RETURNING; default to false.
    else {
        stmt->isReturning = 0;
...
static int dpiStmt__beforeFetch(dpiStmt *stmt, dpiError *error)
{
    ...
    for (i = 0; i < stmt->numQueryVars; i++) {
        ...
        if (!var) {
            ...
            fprintf(stderr, "SHENTONG_DEBUG: dpiStmt__beforeFetch() col %u: oracleTypeNum=%d, defaultNativeTypeNum=%d, clientSizeInBytes=%u\n",
                    i, (int)queryInfo->typeInfo.oracleTypeNum,
                    (int)queryInfo->typeInfo.defaultNativeTypeNum,
...
{
    uint8_t bindNameLengthsBuffer[8] = { 0 }, indNameLengthsBuffer[8] = { 0 }, isDuplicate[8] = { 0 };
