        ...
        dpiError *error)
{
    if (params->driverName && params->driverNameLength > 0) {
        // ShenTong ACI does not support ATTR_DRIVER_NAME (424).
        // Ignore failures so we can still connect.
    ...
    if (params->edition && params->editionLength > 0) {
        // ShenTong ACI may not support ATTR_EDITION either; tolerate failures.
        dpiOci__attrSet(handle, handleType,
                    (void*) params->edition, params->editionLength,
                    DPI_OCI_ATTR_EDITION, NULL, error);
    }
