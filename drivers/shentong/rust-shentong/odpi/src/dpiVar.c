        ...
        int inFetch, dpiError *error)
{
    ...
    if (buffer->returnCode) {
        if (buffer->returnCode[pos] != 0) {
            fprintf(stderr, "SHENTONG_DEBUG: DPI_ERR_COLUMN_FETCH at pos=%u, returnCode=%u\n",
                    pos, (unsigned)buffer->returnCode[pos]);
            fprintf(stderr, "SHENTONG_DEBUG: var oracleTypeNum=%d, nativeTypeNum=%d, sizeInBytes=%llu, maxArraySize=%u\n",
                    (int)var->type->oracleTypeNum, (int)var->nativeTypeNum,
                    (unsigned long long)var->sizeInBytes, buffer->maxArraySize);
            {
                uint32_t dbgIdx;
                fprintf(stderr, "SHENTONG_DEBUG: all returnCodes:");
                for (dbgIdx = 0; dbgIdx < buffer->maxArraySize; dbgIdx++) {
                    fprintf(stderr, " [%u]=%u", dbgIdx, (unsigned)buffer->returnCode[dbgIdx]);
                }
                fprintf(stderr, "\n");
            }
            if (buffer->indicator) {
                fprintf(stderr, "SHENTONG_DEBUG: indicator[%u]=%d\n",
                        pos, (int)buffer->indicator[pos]);
            }
            if (buffer->actualLength16) {
                fprintf(stderr, "SHENTONG_DEBUG: actualLength16[%u]=%u\n",
                        pos, (unsigned)buffer->actualLength16[pos]);
            }
            if (buffer->actualLength32) {
                fprintf(stderr, "SHENTONG_DEBUG: actualLength32[%u]=%u\n",
                        pos, (unsigned)buffer->actualLength32[pos]);
            }
        ...
        dpiError *error)
{
    ...
    if (var->type->defaultNativeTypeNum == DPI_NATIVE_TYPE_BYTES &&
        ...
        if (dpiUtils__allocateMemory(buffer->maxArraySize, sizeof(uint16_t), 1,
