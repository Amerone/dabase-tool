        ...
        dpiError *error)
{
    ...
    // used)
    // Use UTF8 unconditionally.  The original WIN32/WIN64 branch selected
    // ZHS16GBK, but the Windows aci.dll we ship does not support that
    // charset ID correctly in ACINlsCharSetIdToName / ACINlsNameMap, which
    // leads to DPI-1041 errors.  UTF8 is always safe here.
...
static int dpiGlobal__getErrorBuffer(const char *fnName, dpiError *error)
{
    dpiErrorBuffer *tempErrorBuffer = NULL;
    ...
    if (dpiOci__threadKeyGet(dpiGlobalEnvHandle, dpiGlobalErrorHandle,
        ...
        return DPI_FAILURE;
    */
    // aci does not support thread key operations, so fall back to the
    // process-global error buffer.  This is not thread-safe but is
    // sufficient for single-threaded / serialised use.
    if (!tempErrorBuffer)
        tempErrorBuffer = &dpiGlobalErrorBuffer;
...
DPI_INITIALIZER(dpiGlobal__initialize)
{
    ...
    // Always use UTF8; see note in dpiGlobal__extendedInitialize.
        ...
        dpiError *error)
{
    ...
    else if (strcmp(name, DPI_CHARSET_NAME_ZHS16GBK) == 0)
        *charsetId = DPI_CHARSET_ID_ZHS16GBK;
        ...
        dpiError *error)
{
    ...
    switch (charsetId) {
        ...
        case DPI_CHARSET_ID_ASCII:
            ...
            return DPI_SUCCESS;
        case DPI_CHARSET_ID_ZHS16GBK:
            // aci.dll does not export ACINlsNameMap, so bypass ORA->IANA
            // mapping for GBK and return the Oracle name directly.
            strcpy(encoding, DPI_CHARSET_NAME_ZHS16GBK);
            return DPI_SUCCESS;
    }
