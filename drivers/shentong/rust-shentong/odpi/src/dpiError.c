        ...
        const char *action)
{
    ...
    if (dpiOci__errorGet(error->handle, DPI_OCI_HTYPE_ERROR,
        ...
        return DPI_FAILURE;
    fprintf(stderr, "SHENTONG_DEBUG: OCI error code=%d msg='%.*s' (fn=%s action=%s)\n",
            (int)error->buffer->code, error->buffer->messageLength,
            error->buffer->message, error->buffer->fnName, action ? action : "(null)");
    if (dpiDebugLevel & DPI_DEBUG_LEVEL_ERRORS)
    ...
    if (conn && !conn->creating && !conn->deadSession) {
        ...
        if (conn->serverHandle) {
            // Shentong ACI may return garbage values for server status
            // after a SQL error (e.g., 32758 instead of 0 or 1).
            // Only mark dead if get_server_status actually failed OR
            // the status is exactly 0 (OCI_SERVER_NOT_CONNECTED).
