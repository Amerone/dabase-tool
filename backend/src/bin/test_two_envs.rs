...
fn test_two_envs(aci_dir: Option<String>) {
    ...
    type EnvNlsCreate = unsafe extern "C" fn(
        ...
        *mut c_void, *mut c_void, usize, *mut *mut c_void, u16, u16
    ) -> i32;
    type HandleAlloc = unsafe extern "C" fn(
        *mut c_void, *mut *mut c_void, u32, usize, *mut *mut c_void
    ) -> i32;
    type HandleFree = unsafe extern "C" fn(*mut c_void, u32) -> i32;
    type ServerAttach = unsafe extern "C" fn(
        *mut c_void, *mut c_void, *const u8, i32, u32
    ) -> i32;
    type AttrSet = unsafe extern "C" fn(
        *mut c_void, u32, *mut c_void, u32, u32, *mut c_void
    ) -> i32;
    type SessionBegin = unsafe extern "C" fn(
        *mut c_void, *mut c_void, *mut c_void, u32, u32
    ) -> i32;
    type AttrGet = unsafe extern "C" fn(
        *mut c_void, u32, *mut c_void, *mut u32, u32, *mut c_void
    ) -> i32;
    type NlsNumericInfoGet = unsafe extern "C" fn(
        *mut c_void, *mut c_void, *mut i32, u32
    ) -> i32;
    type ErrorGet = unsafe extern "C" fn(
        *mut c_void, u32, *const u8, *mut i32, *mut u8, u32, u32
    ) -> i32;

    let env_nls: EnvNlsCreate = unsafe { std::mem::transmute(get_fn(module, "ACIEnvNlsCreate").unwrap()) };
    let handle_alloc: HandleAlloc = unsafe { std::mem::transmute(get_fn(module, "ACIHandleAlloc").unwrap()) };
    let handle_free: HandleFree = unsafe { std::mem::transmute(get_fn(module, "ACIHandleFree").unwrap()) };
    let server_attach: ServerAttach = unsafe { std::mem::transmute(get_fn(module, "ACIServerAttach").unwrap()) };
    let attr_set: AttrSet = unsafe { std::mem::transmute(get_fn(module, "ACIAttrSet").unwrap()) };
    let session_begin: SessionBegin = unsafe { std::mem::transmute(get_fn(module, "ACISessionBegin").unwrap()) };
    let attr_get: AttrGet = unsafe { std::mem::transmute(get_fn(module, "ACIAttrGet").unwrap()) };
    let nls_info: NlsNumericInfoGet = unsafe { std::mem::transmute(get_fn(module, "ACINlsNumericInfoGet").unwrap()) };
    let error_get: ErrorGet = unsafe { std::mem::transmute(get_fn(module, "ACIErrorGet").unwrap()) };

    let get_error = |err_h: *mut c_void| {
        let mut buf = vec![0u8; 512];
        let mut code: i32 = 0;
        unsafe { error_get(err_h, 1, std::ptr::null(), &mut code, buf.as_mut_ptr(), 512, ACI_HTYPE_ERROR); }
    ...
    println!("\n--- Test 1: ONE env (charset=871, mode=ACI_OBJECT) ---");
    {
        ...
        if rc == 0 && !env_h.is_null() {
            ...
            let rc2 = unsafe { server_attach(srv_h, err_h, cs.as_ptr(), cs.len() as i32, ACI_DEFAULT) };
            println!("  ACIServerAttach: rc={}", rc2);
            if rc2 == 0 { println!("  ONE ENV: SUCCESS"); }
            else { let (code, msg) = get_error(err_h); println!("  ONE ENV: FAIL code={} msg={}", code, msg); }
            unsafe { handle_free(env_h, 1); } // 1 = ACI_HTYPE_ENV
        }
    }
    ...
    println!("\n--- Test 2: GLOBAL env (873, THREADED) + connection env (871, ACI_OBJECT) ---");
    {
        ...
        let rc = unsafe { env_nls(&mut global_env, ACI_THREADED, std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut(), 0, std::ptr::null_mut(), ACI_CHARSET_UTF8_ODPI, ACI_CHARSET_UTF8_ODPI) };
        println!("  Global EnvNlsCreate(873, THREADED): rc={} env={:p}", rc, global_env);
        ...
        if !global_env.is_null() {
            unsafe { handle_alloc(global_env, &mut global_err, ACI_HTYPE_ERROR, 0, std::ptr::null_mut()); }
            println!("  Global error handle: {:p}", global_err);
        }

        // Now create connection env
        let mut conn_env: *mut c_void = std::ptr::null_mut();
        let rc = unsafe { env_nls(&mut conn_env, ACI_OBJECT, std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut(), 0, std::ptr::null_mut(), ACI_CHARSET_UTF8, ACI_CHARSET_UTF8) };
        println!("  Conn EnvNlsCreate(871, ACI_OBJECT): rc={} env={:p}", rc, conn_env);
        if rc == 0 && !conn_env.is_null() {
            let mut err_h: *mut c_void = std::ptr::null_mut();
            let mut srv_h: *mut c_void = std::ptr::null_mut();
            unsafe { handle_alloc(conn_env, &mut err_h, ACI_HTYPE_ERROR, 0, std::ptr::null_mut()); }

            // Extra calls like ODPI does (ACIAttrGet for charset, ACINlsNumericInfoGet)
            let mut charset_id: u16 = 0;
            let rc_attr = unsafe { attr_get(conn_env, 1 /*HTYPE_ENV*/, &mut charset_id as *mut u16 as *mut c_void, std::ptr::null_mut(), 31 /*OCI_ATTR_CHARSET_ID*/, err_h) };
            println!("  ACIAttrGet(CHARSET_ID): rc={} charsetId={}", rc_attr, charset_id);
            let mut max_bytes: i32 = 0;
            let rc_nls = unsafe { nls_info(conn_env, err_h, &mut max_bytes, 91 /*NLS_CHARSET_MAXBYTESZ*/) };
            println!("  ACINlsNumericInfoGet: rc={} maxBytes={}", rc_nls, max_bytes);

            unsafe { handle_alloc(conn_env, &mut srv_h, ACI_HTYPE_SERVER, 0, std::ptr::null_mut()); }
            let cs = connect_str.as_bytes();
            let rc2 = unsafe { server_attach(srv_h, err_h, cs.as_ptr(), cs.len() as i32, ACI_DEFAULT) };
            println!("  ACIServerAttach: rc={}", rc2);
            if rc2 == 0 { println!("  TWO ENVS (THREADED first): SUCCESS"); }
            else { let (code, msg) = get_error(err_h); println!("  TWO ENVS: FAIL code={} msg={}", code, msg); }
        }
    }
    ...
    {
        let mut global_env: *mut c_void = std::ptr::null_mut();
        let rc = unsafe { env_nls(&mut global_env, ACI_THREADED | ACI_OBJECT, std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut(), 0, std::ptr::null_mut(), ACI_CHARSET_UTF8_ODPI, ACI_CHARSET_UTF8_ODPI) };
        println!("  Global EnvNlsCreate(873, THREADED|OBJECT): rc={} env={:p}", rc, global_env);
        ...
        if rc == 0 && !conn_env.is_null() {
            ...
            let rc2 = unsafe { server_attach(srv_h, err_h, cs.as_ptr(), cs.len() as i32, ACI_DEFAULT) };
            println!("  ACIServerAttach: rc={}", rc2);
            if rc2 == 0 { println!("  SUCCESS"); }
            else { let (code, msg) = get_error(err_h); println!("  FAIL code={} msg={}", code, msg); }
        }
    }
    ...
    println!("\n--- Test 4: ONE env, allocate SVCCTX BEFORE SERVER (mirrors working test_aci_ffi) ---");
    {
        ...
        if rc == 0 && !env_h.is_null() {
            ...
            let mut srv_h: *mut c_void = std::ptr::null_mut();
            unsafe { handle_alloc(env_h, &mut err_h, ACI_HTYPE_ERROR, 0, std::ptr::null_mut()); }
            unsafe { handle_alloc(env_h, &mut svc_h, ACI_HTYPE_SVCCTX, 0, std::ptr::null_mut()); } // SVCCTX first!
            unsafe { handle_alloc(env_h, &mut srv_h, ACI_HTYPE_SERVER, 0, std::ptr::null_mut()); }
            let cs = connect_str.as_bytes();
            let rc2 = unsafe { server_attach(srv_h, err_h, cs.as_ptr(), cs.len() as i32, ACI_DEFAULT) };
            println!("  ACIServerAttach: rc={}", rc2);
            if rc2 == 0 { println!("  Test4 (SVCCTX before SERVER, no extra calls): SUCCESS"); }
            else { let (code, msg) = get_error(err_h); println!("  Test4 FAIL code={} msg={}", code, msg); }
        }
    }
    ...
    println!("\n--- Test 5: ONE env, SVCCTX before SERVER + ACIAttrGet + ACINlsNumericInfoGet ---");
    {
        ...
        if rc == 0 && !env_h.is_null() {
            ...
            let mut charset_id: u16 = 0;
            let rc_attr = unsafe { attr_get(env_h, 1 /*HTYPE_ENV*/, &mut charset_id as *mut u16 as *mut c_void, std::ptr::null_mut(), 31 /*ACI_ATTR_CHARSET_ID*/, err_h) };
            println!("  ACIAttrGet(CHARSET_ID): rc={} charsetId={}", rc_attr, charset_id);
            let mut max_bytes: i32 = 0;
            ...
            let rc2 = unsafe { server_attach(srv_h, err_h, cs.as_ptr(), cs.len() as i32, ACI_DEFAULT) };
            println!("  ACIServerAttach: rc={}", rc2);
            if rc2 == 0 { println!("  Test5 (SVCCTX first + extra calls): SUCCESS"); }
            else { let (code, msg) = get_error(err_h); println!("  Test5 FAIL code={} msg={}", code, msg); }
        }
    }
    ...
    println!("\n--- Test 6: ONE env, NO SVCCTX, no extra calls (minimal) ---");
    {
        ...
        if rc == 0 && !env_h.is_null() {
            ...
            let rc2 = unsafe { server_attach(srv_h, err_h, cs.as_ptr(), cs.len() as i32, ACI_DEFAULT) };
            println!("  ACIServerAttach: rc={}", rc2);
            if rc2 == 0 { println!("  Test6 (no SVCCTX, no extra): SUCCESS"); }
            else { let (code, msg) = get_error(err_h); println!("  Test6 FAIL code={} msg={}", code, msg); }
        }
    }
    ...
    println!("\n--- Test 7: Prime with localhost attempt, then try 192.168.3.34 in fresh env ---");
    {
        ...
        let rc = unsafe { env_nls(&mut env_prime, ACI_OBJECT, std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut(), 0, std::ptr::null_mut(), ACI_CHARSET_UTF8, ACI_CHARSET_UTF8) };
        if rc == 0 && !env_prime.is_null() {
            ...
            unsafe { handle_alloc(env_prime, &mut err_prime, ACI_HTYPE_ERROR, 0, std::ptr::null_mut()); }
            unsafe { handle_alloc(env_prime, &mut svc_prime, ACI_HTYPE_SVCCTX, 0, std::ptr::null_mut()); }
            unsafe { handle_alloc(env_prime, &mut srv_prime, ACI_HTYPE_SERVER, 0, std::ptr::null_mut()); }
            let rc2 = unsafe { server_attach(srv_prime, err_prime, prime_str.as_ptr(), prime_str.len() as i32, ACI_DEFAULT) };
            println!("  Prime localhost ACIServerAttach: rc={} (expected -1)", rc2);
        ...
        if rc == 0 && !env_h.is_null() {
            ...
            if rc2 == 0 { println!("  Test7 PRIMING SUCCESS: localhost failure enabled 192.168.3.34!"); }
            else { let (code, msg) = get_error(err_h); println!("  Test7 FAIL code={} msg={}", code, msg); }
        }
    }
