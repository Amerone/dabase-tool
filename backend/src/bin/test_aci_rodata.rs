...
fn attempt(
    ...
    if rc != 0 || env_h.is_null() { println!("  [{}] EnvNlsCreate FAILED rc={}", label, rc); return; }
...
fn test(aci_dir: Option<String>) {
    ...
    let _attr_set: FnAttrSet = unsafe { std::mem::transmute(get_fn(module, "ACIAttrSet").unwrap()) };
    let _session_begin: FnSessBegin = unsafe { std::mem::transmute(get_fn(module, "ACISessionBegin").unwrap()) };
    let _error_get: FnErrGet = unsafe { std::mem::transmute(get_fn(module, "ACIErrorGet").unwrap()) };
    ...
    for connect_str in &["192.168.3.34:2003/osrdb"] {
        ...
        attempt("C", env_nls, handle_alloc, server_attach, bytes.as_ptr(), bytes.len());
    }

    // Test D: attempt() helper with plain byte literal
    println!("\n--- Test D: attempt() with byte literal directly ---");
    attempt("D", env_nls, handle_alloc, server_attach, cs.as_ptr(), cs.len());
    ...
    for connect_str in &["192.168.3.34:2003/osrdb"] {
        ...
        let rc = unsafe { server_attach(srv_h, err_h, cs_bytes.as_ptr(), cs_bytes.len() as i32, 0) };
        println!("  ACIServerAttach: rc={}", rc);
    }
