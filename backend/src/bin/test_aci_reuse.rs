...
fn get_fn(module: *mut c_void, name: &str) -> Option<*mut c_void> {
    ...
    unsafe { let p = GetProcAddress(module, cname.as_ptr() as *const u8); if p.is_null() { None } else { Some(p) } }
}
...
fn run(aci_dir: Option<String>) {
    ...
    println!("\n--- Attempt 1: localhost (prime) using env_h@{:p} ---", &env_h);
    ...
    err_h = std::ptr::null_mut(); svc_h = std::ptr::null_mut(); srv_h = std::ptr::null_mut();
    if rc == 0 && !env_h.is_null() {
        ...
        if rc2 != 0 { let (c, m) = get_error(err_h); println!("  error: code={} '{}'", c, m); }
    }

    println!("\n--- Attempt 2: 192.168.3.34 REUSING same env_h@{:p} ---", &env_h);
    ...
    if rc == 0 && !env_h.is_null() {
        ...
        if rc2 == 0 {
            ...
            unsafe {
                ...
                attr_set(ses_h, 9, u.as_ptr() as *mut c_void, u.len() as u32, 22, err_h);
                attr_set(ses_h, 9, p.as_ptr() as *mut c_void, p.len() as u32, 23, err_h);
