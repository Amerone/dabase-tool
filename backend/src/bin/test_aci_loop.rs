...
#[cfg(windows)]
fn do_connect(
    env_nls: unsafe extern "C" fn(*mut *mut c_void, u32, *mut c_void, *mut c_void, *mut c_void, *mut c_void, usize, *mut *mut c_void, u16, u16) -> i32,
    handle_alloc: unsafe extern "C" fn(*mut c_void, *mut *mut c_void, u32, usize, *mut *mut c_void) -> i32,
    ...
    error_get: unsafe extern "C" fn(*mut c_void, u32, *const u8, *mut i32, *mut u8, u32, u32) -> i32,
    label: &str,
) {
    let mut env_h: *mut c_void = std::ptr::null_mut();
    let rc = unsafe { env_nls(&mut env_h, 2, std::ptr::null_mut(), std::ptr::null_mut(),
        std::ptr::null_mut(), std::ptr::null_mut(), 0, std::ptr::null_mut(), 871u16, 871u16) };
    println!("  [{}] EnvNlsCreate: rc={} env={:p}", label, rc, env_h);
    if rc != 0 || env_h.is_null() { return; }
    ...
    if rc != 0 {
        ...
        unsafe { error_get(err_h, 1, std::ptr::null(), &mut code, buf.as_mut_ptr(), 256, 2); }
        let n = buf.iter().position(|&b| b == 0).unwrap_or(buf.len());
        println!("  [{}] Error: code={} msg={}", label, code, String::from_utf8_lossy(&buf[..n]));
        return;
    }

    unsafe { attr_set(svc_h, 3, srv_h, 0, 6, err_h); }
    let mut ses_h: *mut c_void = std::ptr::null_mut();
    unsafe { handle_alloc(env_h, &mut ses_h, 9, 0, std::ptr::null_mut()); }
    let user = b"sysdba"; let pwd = b"szoscar55";
    unsafe {
        attr_set(ses_h, 9, user.as_ptr() as *mut c_void, user.len() as u32, 22, err_h);
        attr_set(ses_h, 9, pwd.as_ptr() as *mut c_void, pwd.len() as u32, 23, err_h);
        attr_set(svc_h, 3, ses_h, 0, 7, err_h);
    }
    let rc = unsafe { session_begin(svc_h, err_h, ses_h, 1, 0) };
    println!("  [{}] SessionBegin: rc={} => {}", label, rc, if rc == 0 { "SUCCESS" } else { "FAIL" });
}
...
fn test(aci_dir: Option<String>) {
    ...
    type FnEnvNls = unsafe extern "C" fn(*mut *mut c_void, u32, *mut c_void, *mut c_void, *mut c_void, *mut c_void, usize, *mut *mut c_void, u16, u16) -> i32;
    ...
    let handle_alloc: FnAlloc = unsafe { std::mem::transmute(get_fn(module, "ACIHandleAlloc").unwrap()) };
    let server_attach: FnAttach = unsafe { std::mem::transmute(get_fn(module, "ACIServerAttach").unwrap()) };
    let attr_set: FnAttrSet = unsafe { std::mem::transmute(get_fn(module, "ACIAttrSet").unwrap()) };
    let session_begin: FnSessBegin = unsafe { std::mem::transmute(get_fn(module, "ACISessionBegin").unwrap()) };
    let error_get: FnErrGet = unsafe { std::mem::transmute(get_fn(module, "ACIErrorGet").unwrap()) };
    ...
    for _ in 0..1 {
        ...
        if rc != 0 {
            ...
            unsafe { error_get(err_h, 1, std::ptr::null(), &mut code, buf.as_mut_ptr(), 256, 2); }
            let n = buf.iter().position(|&b| b == 0).unwrap_or(buf.len());
            println!("  Error: code={} msg={}", code, String::from_utf8_lossy(&buf[..n]));
        } else {
    ...
    do_connect(env_nls, handle_alloc, server_attach, attr_set, session_begin, error_get, "D");

    println!("\nDone.");
}

#[cfg(not(windows))]
fn test(_: Option<String>) { println!("Windows only"); }
